    ...
    function IEHelper() {
        function getPdfPlugin() {
            ...
            utils.each(classIds, function(index, classId) {
                try {
                    plugin = new window.ActiveXObject(classId);
                    if (plugin) {
                        return false;
                    }
                } catch (ex) {}
...
(function(trv, $, window, document, undefined) {
    ...
    trv.touchBehavior = function(dom, options) {
        ...
        function onGestureStart(e) {
            startDistance = window.kendo.touchDelta(e.touches[0], e.touches[1]).distance;
        }
        function onGestureEnd(e) {}
        function onGestureChange(e) {
            var current = window.kendo.touchDelta(e.touches[0], e.touches[1]).distance;
            onPinch({
                distance: current,
